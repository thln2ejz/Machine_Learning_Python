...
plt.legend()
plt.show()
#variance of coefficient estimates decreases with dataset size; noise increases the variance of the estimates.
...
corrmat = data.drop(['training_set'],axis=1).corr().round(2) #using pandas' corr() 
#corrmat = np.corrcoef(data.drop(['training_set'],axis=1),rowvar=False).round(2) #alternatively using numpy

plt.xticks(ticks=np.arange(len(data.columns)-1),labels=data.columns[:-1],rotation=90)
plt.yticks(ticks=np.arange(len(data.columns)-1),labels=data.columns[:-1])
...
#Exercise 7
X = data[data['training_set']==1].drop(['target','training_set'],axis=1) 
y = data[data['training_set']==1]['target']
...
#colors = cm.jet(np.linspace(0, 1, len(L))) #or below 
...
for i in range(p): 
	...
	#plt.plot(np.log10(L),beta[:,i],c=np.array(colors[i,:]),linewidth=2,label=i+1)
	#plt.scatter(np.log10(L),beta[:,i],c=np.array([colors[i,:]]),linewidth=2)
plt.plot(np.log10(L),np.zeros(len(L)),c='red',linewidth=2) #add x-axis
