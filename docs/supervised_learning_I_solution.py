...
#Exercise 1 
#Analytical solving: Bayes' theorem and p(\omega_2|x) = 1 - p(\omega_1|x)
...
z = [[np.exp(w_0 + w.dot([i,j]))[0] / (1 + np.exp(w_0 + w.dot([i,j])))[0] for i in x1] for j in x2] #nested list comprehension
z = np.array(z) #reshape into ndarray

#from mpl_toolkits.mplot3d import Axes3D  #need this in Jupyter only
...
m1 = X_train.loc[y_train==1].mean() #pd series
...
np.tile(m1,(n_obj,1)) #or any of the below 3 methods
#np.array([m1.values]*100) 
#np.ones((100,1)) * m1.values
#pd.concat([m1]*2,axis=1).T
...
#instead of looping over each object (row of X_test), we use the full data matrix, X_test, and consider the resulting diagonal elements only. 
...
posterior.sum(axis=1) #sanity check
...
#Exercise 4
#Analytical solution derived in the lectures.
...
labels_pred = model.predict(np.c_[xx.ravel(), yy.ravel()]) #or np.stack(..., axis=1)
Z = labels_pred.reshape(xx.shape)

plt.scatter(xx.ravel(), yy.ravel(), c=labels_pred, marker='+', alpha=0.4) 
#plt.contour(xx, yy, Z, alpha=.6, cmap='gray') #filled contour (optionally)
...
model = lda().fit(X, y) #fit entire dataset (find only apparent error later)
y_pred = model.predict(X)
confusion_matrix(y_true=y, y_pred=y_pred)
...
plt.scatter(xx.ravel(), yy.ravel(), c=labels_pred, marker='+', alpha=0.4) 
plt.scatter(X.iloc[:,0], X.iloc[:,1], c=y.replace({1:'black',2:'red'})) #plot this last
plt.show()
...
model = qda().fit(X, y) #fit entire dataset (find only apparent error later)
y_pred = model.predict(X)
confusion_matrix(y_true=y, y_pred=y_pred)
error_apparent = (y_pred!=y).sum() 
...
y_pred = model.predict(X_test)
confusion_matrix(y_true=y_test, y_pred=y_pred)
error_apparent = (y_pred!=y_test).sum() #9 misclassifications
...
for i in dimensionality:
	error_test = [] 
	for j in range(10):
		X1 = np.random.multivariate_normal(mean=np.zeros(i),cov=np.identity(i),size=10+1000) #or cov=np.diag(np.ones(i)) or np.diag([1]*i)
		X2 = np.random.multivariate_normal(mean=np.append(2,np.zeros(i-1)),cov=np.identity(i),size=10+1000) #or mean=[2]+[0]*(i-1) or mean=np.insert(np.zeros(i),0,2)
