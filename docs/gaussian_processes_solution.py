...
#train-validate-test split using np.split (disadvantage: cannot be stratified)
'''
X = pd.DataFrame(np.c_[X_train, y_train]) #alternatively: X = pd.DataFrame({'x':X_train, 'y':y_train}) 

train, validate, test = np.split(X.sample(frac=1), [int(.6*len(X)), int(.8*len(X))])  #60-20-20% split
...
#alternatively: use train_test_split() twice (advantage: can use stratified splitting)
from sklearn.model_selection import train_test_split

X, y = X_train, y_train #start with original dataset as X, y
...
sigma_values = np.linspace(0.3,1,20) #make noise a bit large so the fit isn't too tight
l_values = np.linspace(0.05,1,20)
mse = np.ones([20,20])*np.nan #or np.tile(np.nan,(20,20)) or np.full([20,20], np.nan)
...
#alternatively: using list append and reshaping at the end
'''
from itertools import product

mse, mae = [], []
counter = 0
for sigma_noise, l in product(np.linspace(0.05,1,20), np.linspace(0.05,1,20)):
	print(f'countdown:...{20*20-counter}')
	k_xx = kernel_rbf(X_train, X_train, l)
	k_xxs = kernel_rbf(X_train, X_val, l)
	k_xsx = kernel_rbf(X_val, X_train, l)
	k_xsxs = kernel_rbf(X_val, X_val, l)
	...
	mse.append(((y_pred-y_val)**2).mean())
	mae.append(np.median(abs(y_pred-y_val)))
	counter += 1

mse = np.array(mse).reshape(20,20)
mae = np.array(mae).reshape(20,20)
'''

row, col = np.unravel_index(mse.argmin(), mse.shape) #or np.where(mse == np.min(mse)) #or np.nanmin(mse) to ignore any nans
...
#test set evaluation
k_xx = kernel_rbf(np.r_[X_train,X_val], np.r_[X_train,X_val], l_opt) #or np.hstack([X_train,X_val])
k_xxs = kernel_rbf(np.r_[X_train,X_val], X_test, l_opt) #n x number of domain points
...
#kernel = ExpSineSquared(1.0, 5.0, periodicity_bounds=(1e-2, 1e1)) + WhiteKernel(1e-1) #works well too 
kernel = RBF() #1.0 * RBF(length_scale=1.0, length_scale_bounds=(0.05, 1))
y_pred_prior = GaussianProcessRegressor(kernel=kernel).sample_y(X_test.reshape(-1,1), n_samples=n_samples) #sample from pior
model = GaussianProcessRegressor(kernel=kernel, alpha=0.5).fit(X.reshape(-1,1), y) #alpha is sigma_noise (needs to be set)
